...
1. **幂等**：申请带客户端生成的 `requestId`，`dispatch_order.request_id` 唯一键兜底。重复提交（含同 requestId 并发）只返回原单，绝不二次占库。
2. **行锁串行化争抢**：占用/出库/回补在事务内先 `SELECT ... FOR UPDATE` 按批次 id 升序锁定相关批次，多个避难点抢同一批次时在此排队，统一加锁顺序避免死锁。
3. **条件 UPDATE 双保险**：真正增减用 `UPDATE ... SET held=held+? WHERE id=? AND quantity-held>=?` 这类条件语句，影响行数必须为 1，否则整单回滚——从机制上杜绝超占、超扣、凭空回补。
4. **单事务原子性**：库存动作 + 状态 CAS（`WHERE status = 期望值`）+ 批次去向 + 库存流水 + 状态时间线 + 审计同提交同回滚，失败不留半占用。
